...
gainDefense='&d+#',loseDefense='&d-(#+1)',
chance='500/((#*(1/(@/2)))*(@/2))',  -- Eu fiz essa equação para variar de +0 a +7 o item
--- Essa equação deve retornar em % a chance do item se refinar (0-100) 100 = sempre, 0 = nunca
maxlvl = 9,
blocked_ids = {2488,2640}
}
-- &a = weapon attack
-- &d = weapon defense
-- &s = shield defense
...
function onUse(cid, item, fromPosition, itemEx, toPosition)
           ...
           or (getItemWeaponType(itemEx.uid) == 0
            and isArmor(itemEx) == FALSE)
             or itemEx.itemid == 0 then
                doPlayerSendTextMessage(cid, 24,"Seu item ja esta no level maximo, nao da mais para refinar.")
                return TRUE
         end
        ...
        if chance >= math.random(0,100) or item.actionid >= 1000 or (item.actionid == 500 and math.random(0,100) <= 25) then
           if level+1 > gain.maxlvl then
              ...
              return doPlayerSendTextMessage(cid, 24,"Seu item ja esta no level maximo, nao da mais para refinar.")
           else
              setItemName(itemEx.uid, getItemNameById(itemEx.itemid)..' +'..(level+1))
              doPlayerSendTextMessage(cid, 24,"Seu item evoluiu para +"..(level+1)..".")
           ...
           elseif level == gain.maxlvl then
                  doSendMagicEffect(toPosition, 2)
                  return doPlayerSendTextMessage(cid, 24,"Seu item esta level maximo, nao da mais para refinar.")
           elseif level > 0 then
               if level == 1 then
